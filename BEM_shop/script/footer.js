'use strict';
let footerEnd = document.querySelector(".footer-end");
let footerEndInner = `
<span class="footer-end__copy-right">&copy; 2017 Brand All Rights Reserved</span>
<div class="footer-end__socials">
    <i class="footer-end__social-item fab fab fa-facebook-f"></i> 
    <i class="footer-end__social-item fab fa-twitter"></i>
    <i class="footer-end__social-item fab fa-instagram"></i>
    <i class="footer-end__social-item fab fa-pinterest-p"></i>
    <i class="footer-end__social-item fab fa-google-plus-g"></i>
</div>
`;
footerEnd.insertAdjacentHTML('beforeend', footerEndInner);

let footerMain = document.querySelector(".footer-main");
let footerMainInner = `
    <div class="footer-main__text">
        <a class="logo header__logo" href="#">
            <svg
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                width="59px" height="51px">
                <image  x="0px" y="0px" width="59px" height="51px"  xlink:href="data:img/png;base64," />
            </svg>

            <span class="logo__letters">BRAN<span class="logo__special-letter">D</span></span>
        </a>
        
        <p class="footer-main__info">Objectively transition extensive data rather than cross functional solutions. Monotonectally syndicate multidisciplinary materials before go&nbsp;forward benefits. Intrinsicly syndicate an&nbsp;expanded array of&nbsp;processes and cross-unit partnerships.</p>
        <p class="footer-main__info">Efficiently plagiarize 24/365 action items and focused infomediaries. Distinctively seize superior initiatives for wireless technologies. Dynamically optimize.</p>
    </div>
    <div class="footer-main__menu-wrap">
        <div class="footer-main__menu">
            <span class="footer-main__header">COMPANY</span>
            <a href="index.html" class="footer-main__item">Home</a>
            <a href="men.html" class="footer-main__item">Shop</a>
            <a href="#" class="footer-main__item">About</a>
            <a href="#" class="footer-main__item">How It Works</a>
            <a href="#" class="footer-main__item">Contact</a>
        </div>
        <div class="footer-main__menu">
            <span class="footer-main__header">INFORMATION</span>
            <a href="#" class="footer-main__item">Tearms & Condition</a>
            <a href="#" class="footer-main__item">Privacy Policy</a>
            <a href="#" class="footer-main__item">How to Buy</a>
            <a href="#" class="footer-main__item">How to Sell</a>
            <a href="#" class="footer-main__item">Promotion</a>
        </div>
            <div class="footer-main__menu">
            <span class="footer-main__header">SHOP CATEGORY</span>
            <a href="men.html" class="footer-main__item">Men</a>
            <a href="#" class="footer-main__item">Women</a>
            <a href="#" class="footer-main__item">Child</a>
            <a href="#" class="footer-main__item">Apparel</a>
            <a href="#" class="footer-main__item">Browse All</a>
            </div>
        </div>
`;
footerMain.insertAdjacentHTML('beforeend', footerMainInner);